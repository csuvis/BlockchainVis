...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for silubiumd is necessary for this upgrade with the "reindex=1" bitcoin.conf option. \n' +
                 'There are changes necessary in both bitcoin.conf and silubiumcore-node.json. \n\n' +
 ...
 * @param {Object} options.config - The parsed silubiumcore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './silubiumcore-node.json');
  ...
  node.on('ready', function() {
    log.info('Silubiumcore Node ready');
  });
...
function loadModule(req, service) {
  try {
    // first try in the built-in silubiumcore-node services directory
    ...
    if (servicePackage.silubiumcoreNode) {
      serviceModule = service.name + '/' + servicePackage.silubiumcoreNode;
    }
