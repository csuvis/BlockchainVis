...
var BASE_PACKAGE = {
  ...
  dependencies: {
    ...
    'silubiumcore-lib': 'git+http://172.16.0.99/szc/silubiumcore-lib.git#master',
    'silubiumcore-node': 'git+http://172.16.0.99/szc/silubiumcore-node.git#master'
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/silubiumcore-node.json', configJSON);
      if (!isGlobal) {
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the silubiumcore-node directory and configuration
