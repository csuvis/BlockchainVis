...
var SilubiumRPC = require('silubiumd-rpc');
...
/**
 * Provides a friendly event driven API to silubiumd in Node.js. Manages starting and
 * stopping silubiumd as a child process for application support, as well as connecting
 * to multiple silubiumd processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function Silubium(options) {
    if (!(this instanceof Silubium)) {
        return new Silubium(options);
    }
...
util.inherits(Silubium, Service);

Silubium.dependencies = [];

Silubium.DEFAULT_MAX_TXIDS = 1000;
Silubium.DEFAULT_MAX_HISTORY = 50;
Silubium.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
Silubium.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
Silubium.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
Silubium.DEFAULT_SPAWN_RESTART_TIME = 5000;
Silubium.DEFAULT_SPAWN_STOP_TIME = 10000;
Silubium.DEFAULT_TRY_ALL_INTERVAL = 1000;
Silubium.DEFAULT_REINDEX_INTERVAL = 10000;
Silubium.DEFAULT_START_RETRY_INTERVAL = 5000;
Silubium.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
Silubium.DEFAULT_TRANSACTION_CONCURRENCY = 5;
Silubium.DEFAULT_CONFIG_SETTINGS = {
    server: 1,
    whitelist: '127.0.0.1',
    txindex: 1,
    addressindex: 1,
...
Silubium.prototype._initDefaults = function (options) {
    /* jshint maxcomplexity: 15 */

    // limits
    this.maxTxids = options.maxTxids || Silubium.DEFAULT_MAX_TXIDS;
    this.maxTransactionHistory = options.maxTransactionHistory || Silubium.DEFAULT_MAX_HISTORY;
    this.maxAddressesQuery = options.maxAddressesQuery || Silubium.DEFAULT_MAX_ADDRESSES_QUERY;
    this.shutdownTimeout = options.shutdownTimeout || Silubium.DEFAULT_SHUTDOWN_TIMEOUT;

    // spawn restart setting
    this.spawnRestartTime = options.spawnRestartTime || Silubium.DEFAULT_SPAWN_RESTART_TIME;
    this.spawnStopTime = options.spawnStopTime || Silubium.DEFAULT_SPAWN_STOP_TIME;

    // try all interval
    this.tryAllInterval = options.tryAllInterval || Silubium.DEFAULT_TRY_ALL_INTERVAL;
    this.startRetryInterval = options.startRetryInterval || Silubium.DEFAULT_START_RETRY_INTERVAL;

    // rpc limits
    this.transactionConcurrency = options.transactionConcurrency || Silubium.DEFAULT_TRANSACTION_CONCURRENCY;

    // sync progress level when zmq subscribes to events
    this.zmqSubscribeProgress = options.zmqSubscribeProgress || Silubium.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

Silubium.prototype._initCaches = function () {
...
Silubium.prototype._initClients = function () {
    var self = this;
    this.nodes = [];
    this.nodesIndex = 0;
 ...
 */
Silubium.prototype.getAPIMethods = function () {
    var methods = [
 ...
 */
Silubium.prototype.getPublishEvents = function () {
    return [
        {
...
Silubium.prototype.subscribe = function (name, emitter) {
    this.subscriptions[name].push(emitter);
    log.info(emitter.remoteAddress, 'subscribe:', 'silubiumd/' + name, 'total:', this.subscriptions[name].length);
};

Silubium.prototype.unsubscribe = function (name, emitter) {
    var index = this.subscriptions[name].indexOf(emitter);
    if (index > -1) {
        this.subscriptions[name].splice(index, 1);
    }
    log.info(emitter.remoteAddress, 'unsubscribe:', 'silubiumd/' + name, 'total:', this.subscriptions[name].length);
};


Silubium.prototype.subscribeBalance = function (emitter, addresses) {
    var self = this;
    ...
    log.info(emitter.remoteAddress, 'subscribe:', 'silubiumd/addressbalance', 'total:', _.size(this.subscriptions.balance));
};

Silubium.prototype.unsubscribeBalance = function (emitter, addresses) {
    ...
    log.info(emitter.remoteAddress, 'unsubscribe:', 'silubiumd/addressbalance', 'total:', _.size(this.subscriptions.balance));
};

Silubium.prototype.unsubscribeBalanceAll = function (emitter) {
...
Silubium.prototype.subscribeAddress = function (emitter, addresses) {
    var self = this;
    ...
    log.info(emitter.remoteAddress, 'subscribe:', 'silubiumd/addresstxid', 'total:', _.size(this.subscriptions.address));
};

Silubium.prototype.unsubscribeAddress = function (emitter, addresses) {
    var self = this;
    if (!addresses) {
    ...
    log.info(emitter.remoteAddress, 'unsubscribe:', 'silubiumd/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
Silubium.prototype.unsubscribeAddressAll = function (emitter) {
...
Silubium.prototype._getDefaultConfig = function () {
    var config = '';
    var defaults = Silubium.DEFAULT_CONFIG_SETTINGS;
...
Silubium.prototype._parseBitcoinConf = function (configPath) {
    var options = {};
...
Silubium.prototype._expandRelativeDatadir = function () {
...
Silubium.prototype._loadSpawnConfiguration = function (node) {
    /* jshint maxstatements: 25 */

    $.checkArgument(this.options.spawn, 'Please specify "spawn" in silubiumd config options');
    $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in silubiumd config options');
    $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in silubiumd config options');

    this._expandRelativeDatadir();

    var spawnOptions = this.options.spawn;
    var configPath = path.resolve(spawnOptions.datadir, './silubium.conf');

    log.info('Using silubium config file:', configPath);
...
Silubium.prototype._checkConfigIndexes = function (spawnConfig, node) {
    $.checkState(
        spawnConfig.txindex && spawnConfig.txindex === 1,
        '"txindex" option is required in order to use transaction query features of silubiumcore-node. ' +
    ...
    $.checkState(
        ...
        '"addressindex" option is required in order to use address query features of silubiumcore-node. ' +
    ...
    $.checkState(
        ...
        '"spentindex" option is required in order to use spent info query features of silubiumcore-node. ' +
    ...
    $.checkState(
        ...
        '"server" option is required to communicate to silubiumd from bitcore. ' +
        'Please add "server=1" to your configuration and restart'
    );

    $.checkState(
        spawnConfig.zmqpubrawtx,
        '"zmqpubrawtx" option is required to get event updates from silubiumd. ' +
        'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
    );

    $.checkState(
        spawnConfig.zmqpubhashblock,
        '"zmqpubhashblock" option is required to get event updates from silubiumd. ' +
    ...
    if (spawnConfig.reindex && spawnConfig.reindex === 1) {
        log.warn('Reindex option is currently enabled. This means that silubiumd is undergoing a reindex. ' +
            ...
            'of silubiumcore-node services will start.');
        node._reindex = true;
    }
};

Silubium.prototype._resetCaches = function () {
    ...
    this.transactionReceiptCache.reset();
    this.dgpInfoCache = null;
    this.miningInfoCache = null;
    this.stakingInfoCache = null;
};

Silubium.prototype._tryAllClients = function (func, callback) {
    var self = this;
...
Silubium.prototype._wrapRPCError = function (errObj) {
    var err = new errors.RPCError(errObj.message);
    err.code = errObj.code;
    return err;
};

Silubium.prototype._initChain = function (callback) {
    var self = this;
        ...
        self.client.getBlock(response.result, function (err, response) {
            ...
            self.client.getBlockHash(0, function (err, response) {
                ...
                self.getRawBlock(blockhash, function (err, blockBuffer) {
                    ...
                    self.emit('ready');
                    log.info('Silubium Daemon Ready');
                    callback();
                });
            });
...
Silubium.prototype._getDefaultConf = function () {
    var networkOptions = {
        rpcport: 8332
    };
...
Silubium.prototype._getNetworkConfigPath = function () {
    var networkPath;
    if (this.node.network === bitcore.Networks.testnet) {
        networkPath = 'testnet3/silubium.conf';
        if (this.node.network.regtestEnabled) {
            networkPath = 'regtest/silubium.conf';
        }
    }
    return networkPath;
};

Silubium.prototype._getNetworkOption = function () {
    var networkOption;
    if (this.node.network === bitcore.Networks.testnet) {
...
Silubium.prototype._zmqBlockHandler = function (node, message) {
    var self = this;
    ...
    if (!self.zmqKnownBlocks.get(id)) {
        ...
        for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
            this.subscriptions.hashblock[i].emit('silubiumd/hashblock', message.toString('hex'));
        }
    }

};

Silubium.prototype._rapidProtectedUpdateTip = function (node, message) {
    var self = this;
...
Silubium.prototype._updateTip = function (node, message) {
    var self = this;
        ...
        if (!self.node.stopping) {
            self.syncPercentage(function (err, percentage) {
                if (err) {
                    ...
                    log.info('Silubium Height:', self.height, 'Percentage:', percentage.toFixed(2));
                }
            });
        }
    }
...
Silubium.prototype._getAddressesFromTransaction = function (transaction) {
    var addresses = [];
...
Silubium.prototype._notifyBalanceSubscribers = function (txid, transaction) {
...
Silubium.prototype._notifyBalanceSubscriber = function (address, txid) {
    var self = this;
    self.getAddressSummary(address, {noTxList: false}, function (err, data) {
        ...
        if (emitters) {
            for (var j = 0; j < emitters.length; j++) {
                emitters[j].emit('silubiumd/addressbalance', {
                    address: address,
                    txid: txid,
...
Silubium.prototype._notifyAddressTxidSubscribers = function (txid, transaction) {
    var addresses = this._getAddressesFromTransaction(transaction);
    for (var i = 0; i < addresses.length; i++) {
        var address = addresses[i];
        if (this.subscriptions.address[address]) {
            ...
            for (var j = 0; j < emitters.length; j++) {
                emitters[j].emit('silubiumd/addresstxid', {
                    address: address,
                    txid: txid
                });
            }
...
Silubium.prototype._zmqTransactionHandler = function (node, message) {
    ...
    if (!self.zmqKnownTransactions.get(id)) {
        ...
        for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
            this.subscriptions.rawtransaction[i].emit('silubiumd/rawtransaction', message.toString('hex'));
        }
...
Silubium.prototype._checkSyncedAndSubscribeZmqEvents = function (node) {
    var self = this;
    var interval;
    ...
    checkAndSubscribe(function (err, synced) {
        ...
        if (!synced) {
            ...
            }, node._tipUpdateInterval || Silubium.DEFAULT_TIP_UPDATE_INTERVAL);
        }
    });

};

Silubium.prototype._subscribeZmqEvents = function (node) {
    var self = this;
...
Silubium.prototype._initZmqSubSocket = function (node, zmqUrl) {
...
Silubium.prototype._checkReindex = function (node, callback) {
    var self = this;
    var interval;
    ...
    if (node._reindex) {
        interval = setInterval(function () {
            node.client.getBlockchainInfo(function (err, response) {
                ...
                log.info('Silubium Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
        ...
        }, node._reindexWait || Silubium.DEFAULT_REINDEX_INTERVAL);
    } else {
        callback();
    }
};

Silubium.prototype._loadTipFromNode = function (node, callback) {
    var self = this;
    node.client.getBestBlockHash(function (err, response) {
        if (err && err.code === -28) {
...
Silubium.prototype._stopSpawnedBitcoin = function (callback) {
    var self = this;
    var spawnOptions = this.options.spawn;
    var pidPath = spawnOptions.datadir + '/silubiumd.pid';
...
Silubium.prototype._spawnChildProcess = function (callback) {
    var self = this;
    ...
    self._stopSpawnedBitcoin(function (err) {
        ...
        log.info('Starting silubium process');
        ...
        self.spawn.process.once('exit', function (code) {
            if (!self.node.stopping) {
                log.warn('Silubium process unexpectedly exited with code:', code);
                log.warn('Restarting silubium child process in ' + self.spawnRestartTime + 'ms');
                setTimeout(function () {
                    self._spawnChildProcess(function (err) {
                        if (err) {
                            return self.emit('error', err);
                        }
                        log.warn('Silubium process restarted');
                    });
        ...
        async.retry({times: 60, interval: self.startRetryInterval}, function (done) {
            ...
            if (exitShutdown) {
                return callback(new Error('Stopping while trying to spawn silubiumd.'));
            }
...
Silubium.prototype._connectProcess = function (config, callback) {
    var self = this;
    var node = {};
    var exitShutdown = false;
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function (done) {
        ...
        if (exitShutdown) {
            return callback(new Error('Stopping while trying to connect to silubiumd.'));
        }
 ...
 */
Silubium.prototype.start = function (callback) {
    var self = this;
    ...
    async.series([
        ...
        if (self.nodes.length === 0) {
            return callback(new Error('Silubium configuration options "spawn" or "connect" are expected'));
        }
 ...
 * @param {Function} callback
 */
Silubium.prototype.isSynced = function (callback) {
 ...
 * @param {Function} callback
 */
Silubium.prototype.syncPercentage = function (callback) {
    var self = this;
...
Silubium.prototype._normalizeAddressArg = function (addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, silubiumcore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
Silubium.prototype.getAddressBalance = function (addressArg, options, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Silubium.prototype.getAddressUnspentOutputs = function (addressArg, options, callback) {
    var self = this;
...
Silubium.prototype._getBalanceFromMempool = function (deltas) {
    var satoshis = 0;
...
Silubium.prototype._getTxidsFromMempool = function (deltas) {
    var mempoolTxids = [];
...
Silubium.prototype._getHeightRangeQuery = function (options, clone) {
    if (options.start >= 0 && options.end >= 0) {
        if (options.end > options.start) {
            throw new TypeError('"end" is expected to be less than or equal to "start"');
        }
        if (clone) {
            // reverse start and end as the order in silubiumcore is most recent to less recent
 ...
 * @param {Function} callback
 */
Silubium.prototype.getAddressTxids = function (addressArg, options, callback) {
    /* jshint maxstatements: 20 */
    var self = this;
    var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
...
Silubium.prototype._getConfirmationsDetail = function (transaction) {
...
Silubium.prototype._getAddressDetailsForInput = function (input, inputIndex, result, addressStrings) {
    if (!input.address) {
        return;
    }
...
Silubium.prototype._getAddressDetailsForOutput = function (output, outputIndex, result, addressStrings) {
    if (!output.address) {
        return;
    }
...
Silubium.prototype._getAddressDetailsForTransaction = function (transaction, addressStrings) {
    var result = {
        addresses: {},
        satoshis: 0
    };
 ...
 * @param {Object} txid - A silubiumcoin transaction id
 * @param {Function} callback
 */
Silubium.prototype._getAddressDetailedTransaction = function (txid, options, next) {
    var self = this;
...
Silubium.prototype._getAddressStrings = function (addresses) {
    var addressStrings = [];
    for (var i = 0; i < addresses.length; i++) {
        var address = addresses[i];
...
Silubium.prototype._paginateTxids = function (fullTxids, fromArg, toArg) {
    var txids;
 ...
 * @param {Function} callback
 */
Silubium.prototype.getAddressHistory = function (addressArg, options, callback) {
    var self = this;
    var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Silubium.prototype.getAddressesMempoolBalance = function (addressArg, options, callback) {
    var self = this;
    var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Silubium.prototype.getAddressSummary = function (addressArg, options, callback) {
    var self = this;
    var summary = {};
    var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
    var summaryTxids = [];
...
Silubium.prototype._maybeGetBlockHash = function (blockArg, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Silubium.prototype.getRawBlock = function (blockArg, callback) {
 ...
 * @param {Function} callback
 */
Silubium.prototype.getBlockOverview = function (blockArg, callback) {
    var self = this;
...
Silubium.prototype.getJsonBlock = function (blockArg, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Silubium.prototype.getBlock = function (blockArg, callback) {
    // TODO apply performance patch to the RPC method for raw data
    var self = this;
 ...
 * @param {Function} callback
 */
Silubium.prototype.getBlockHashesByTimestamp = function (high, low, options, callback) {
 ...
 * @param {Function} callback
 */
Silubium.prototype.getBlockHeader = function (blockArg, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Silubium.prototype.estimateFee = function (blocks, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Silubium.prototype.sendTransaction = function (tx, options, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Silubium.prototype.getRawTransaction = function (txid, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Silubium.prototype.getJsonRawTransaction = function (txid, callback) {
    var self = this;
...
/**
 * Will get a transaction as a Silubiumcore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
Silubium.prototype.getTransaction = function (txid, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Silubium.prototype.getDetailedTransaction = function (txid, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Silubium.prototype.getBestBlockHash = function (callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Silubium.prototype.getSpentInfo = function (options, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Silubium.prototype.getInfo = function (callback) {
...
Silubium.prototype.generateBlock = function (num, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Silubium.prototype.stop = function (callback) {
    if (this.spawn && this.spawn.process) {
        ...
        this.spawn.process.once('exit', function (code) {
            if (!exited) {
                exited = true;
                if (code !== 0) {
                    var error = new Error('silubiumd spawned process exited with status code: ' + code);
                    error.code = code;
        ...
        setTimeout(function () {
            if (!exited) {
                exited = true;
                return callback(new Error('silubiumd process did not exit'));
            }
 ...
 * @param {Function} callback
 */
Silubium.prototype.listUnspent = function (minConf, maxConf, addresses, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Silubium.prototype.getNewAddress = function (callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Silubium.prototype.callContract = function (address, hash, options, callback) {
    var self = this,
 ...
 * @param {Function} callback
 */
Silubium.prototype.getAccountInfo = function (address, callback) {
 ...
 * @param {Function} callback
 */
Silubium.prototype.getSubsidy = function (height, callback) {
 ...
 * @param {Function} callback
 */
Silubium.prototype.getTransactionReceipt = function (trxHash, callback) {
 ...
 * @param {Function} callback
 */
Silubium.prototype.getDgpInfo = function (callback) {
 ...
 * @return {*}
 */
Silubium.prototype.getMiningInfo = function (callback) {
    var self = this,
 ...
 * @return {*}
 */
Silubium.prototype.getStakingInfo = function (callback) {
    var self = this,
