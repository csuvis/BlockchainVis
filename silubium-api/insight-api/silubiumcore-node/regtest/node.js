...
describe('Node Functionality', function() {
    ...
    rimraf(datadir + '/regtest', function(err) {
      ...
      node.start(function(err) {
        ...
        var syncedHandler = function() {
          if (node.services.silubiumd.height === 150) {
            node.services.silubiumd.removeListener('synced', syncedHandler);
            done();
          }
        };

        node.services.silubiumd.on('synced', syncedHandler);
  ...
  describe('Bus Functionality', function() {
    it('subscribes and unsubscribes to an event on the bus', function(done) {
      ...
      bus.on('silubiumd/hashblock', function(data) {
        bus.unsubscribe('silubiumd/hashblock');
        if (blockExpected) {
  ...
  describe('Address Functionality', function() {
    ...
    before(function(done) {
      ...
      var startHeight = node.services.silubiumd.height;
      node.services.silubiumd.on('tip', function(height) {
    ...
    describe('Mempool Index', function() {
      ...
      it('will update the mempool index after new tx', function(done) {
        ...
        node.services.silubiumd.sendTransaction(tx.serialize(), function(err, hash) {
