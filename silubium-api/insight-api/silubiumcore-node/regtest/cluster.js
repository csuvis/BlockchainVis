...
describe('Bitcoin Cluster', function() {
  var node;
  var daemons = [];
  var execPath = path.resolve(__dirname, '../bin/silubiumd');
  var nodesConf = [
    {
  ...
  before(function(done) {
    log.info('Starting 3 silubiumd daemons');
    this.timeout(60000);
  ...
  it('step 1: will connect to three silubiumd daemons', function(done) {
    this.timeout(20000);
    var configuration = {
      network: 'regtest',
      services: [
  ...
  it('step 2: receive block events', function(done) {
    this.timeout(10000);
    node.services.silubiumd.once('tip', function(height) {
