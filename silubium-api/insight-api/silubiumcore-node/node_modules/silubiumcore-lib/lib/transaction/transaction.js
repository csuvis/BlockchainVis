...
Transaction.prototype.toBufferWriter = function(writer) {
  ...
  if (this.flags) {
    ...
    for (var j = 0; j < this.inputs.length; j++) {
      ...
      if (this.witnessStack[j].length) {
          for (var k = 0; k < this.witnessStack[j].length; k++) {
              writer.writeVarintNum(this.witnessStack[j][k].length);
              writer.write(this.witnessStack[j][k]);
          }
      }
