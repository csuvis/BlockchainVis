$(function () {
    ...
    //地址的交易列表
    function addrtxslistTable(data) {
        // console.log(data);
    ...
    function tjsvgrender0(result) {
        ...
        var tjbpos = {
            ...
            y2x: 0.975 * svgw  //y2轴位置x
        }
        // 显示图例
        var tjbsign = mainsvg.append("g");
        tjbsign.append("rect").attr("x", tjbpos.xx + 0.32 * svgw).attr("y", 0.045 * svgh)
            .attr("class", "line3-text").attr('width', 0.02 * svgw).attr('height', 0.001 * svgw);
        tjbsign.append("circle").attr("cx", tjbpos.xx + 0.33 * svgw).attr("cy", 0.045 * svgh + 0.0005 * svgw)
            .attr('r', 0.002 * svgw).attr("class", "line3-text");
        tjbsign.append("text").attr("x", tjbpos.xx + 0.35 * svgw).attr("y", 0.055 * svgh)
            .attr("class", "legend").text($.i18n.prop("ad-FinalBalance")).style('font-family','sans-serif').style("font-size", 0.75*autofontsize + "px");

        tjbsign.append("text").attr("x", tjbpos.xx + 0.52 * svgw).attr("y", 0.055 * svgh)
            .attr("class", "bar1-rect").text("■").style("font-size", 0.75*autofontsize + "px");// reward_sum
        tjbsign.append("text").attr("x", tjbpos.xx + 0.535 * svgw).attr("y", 0.055 * svgh)
            .attr("class", "legend").text($.i18n.prop("svgTrans")).style('font-family','sans-serif')
        ...
        mainsvg.append("g")
            ...
            .style("font-size", 0.65 * autofontsize + "px");
        mainsvg.append("text")
            .attr("transform", "translate(" + (tjbpos.y2x+0.025*svgw) + "," + (tjbpos.yy - 0.03 * svgh) + ")")
            .text($.i18n.prop("svgTrans")).style('font-family','sans-serif')
            .style("font-size", 0.75*autofontsize + "px")
        ...
        mainsvg.append("text")
            ...
            .text($.i18n.prop("ad-FinalBalance")).style('font-family','sans-serif')
            .style("font-size", 0.75*autofontsize + "px")
