...
(function($) {
  ...
  $.i18n.properties = function(settings) {
    ...
    // Ensure an array
    var files =
      settings.name && settings.name.constructor === Array
        ? settings.name
        : [settings.name];
    ...
    settings.totalFiles =
      files.length * 2 + (settings.language.length >= 5 ? files.length : 0);
    if (settings.debug) {
          ...
          if (settings.mode == "vars" || settings.mode == "both") {
            ...
            if (regPlaceHolder.test(value)) {
              ...
              parts.forEach(function(part) {
                if (
                  regPlaceHolder.test(part) &&
                  (usedArgs.length === 0 || usedArgs.indexOf(part) == -1)
                ) {
                  if (!first) {
                    fnArgs += ",";
                  }
  ...
  $.i18n.normaliseLanguageCode = function(settings) {
    ...
    if (!lang || lang.length < 2) {
      if (settings.debug)
        debug("No language supplied. Pulling it from the browser ...");
      lang =
