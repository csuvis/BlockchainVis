...
console.log(result);
                ...
                addrsLinear = d3.scaleLinear()
                    .domain([1, 1.5 * addrsScale_max]) //取输入输出地址数量的最大值
    ...
    function txsrender(data) {
        // console.log(data);
        $("#txlistTable").bootstrapTable('destroy');
        $('#txlistTable').bootstrapTable({
            data: data,
            pagination: true,
            sortable: true,
            columns: [{
            ...
            onSort: function (name, order) {
                ...
                $("#coin_filter").mask();
                // coinfilterrender(resultdata,radioitem); //铜钱选择区
                initcoinrender(data, name, order);
                $("#coin_filter").unmask();
            }
        });
    }
 ...
 var  flag_addr=true;var XMAX_1; //控制地址数量chart的横轴不变
    function coinfilterrender1(result) {  //地址数量图
        ...
        mainfilter.append("g")
            ...
            .call(xAxis).style('font-size',0.6*autofontsize+'px')
            .selectAll("text");
            // .style("font-size", 0.65 * autofontsize + "px");

        mainfilter.append("text").attr('text-anchor', 'end')
            .attr("transform", "translate(" + (xsize.w + xsize.x) + "," + (xsize.y + padding.top * 2) + ")rotate(0)")
            .text($.i18n.prop("axis-x1"))
            .style("font-size", 0.9*autofontsize + "px");
        ...
        mainfilter.append("g")
            ...
            .call(yAxis).style('font-size',0.6*autofontsize+'px');
            // .style("font-size", 0.7 * autofontsize + "px");
        mainfilter.append("text")
            .attr("transform", "translate(" + (ysize.x - 20) + "," + (ysize.y - 15) + ")rotate(0)")
...
// console.log(flag_fee)
        ...
        mainfilter2.append("g")
            .attr("class", "axis xAxis")
            .attr("transform", "translate(" + xsize.x + "," + xsize.y + ")")
            .call(xAxis2).style('font-size',0.6*autofontsize+'px')
            .selectAll("text");

        mainfilter2.append("text").attr('text-anchor', 'end')
            .attr("transform", "translate(" + (xsize.w + 1.12 * xsize.x) + "," + (xsize.y + padding.top * 2) + ")rotate(0)")
        ...
        mainfilter2.append("g")
            .attr("class", "axis xAxis")
            .attr("transform", "translate(" + ysize.x + "," + ysize.y + ")")
            .call(yAxis2).style('font-size',0.6*autofontsize+'px');
        mainfilter2.append("text")
            .attr("transform", "translate(" + (ysize.x - 23) + "," + (ysize.y - 15) + ")rotate(0)")
            .text($.i18n.prop("axis-txs"))
            .style("font-size", 0.9*autofontsize + "px");
        //刷选费用
    ...
    function coinfilterrender3(result) {
        ...
        mainfilter3.append("g")
            .attr("class", "axis xAxis")
            .attr("transform", "translate(" + xsize.x + "," + xsize.y + ")")
            .call(xAxis3).style('font-size',0.6*autofontsize+'px')
            .selectAll("text");
        ...
        mainfilter3.append("g")
            .attr("class", "axis xAxis")
            .attr("transform", "translate(" + ysize.x + "," + ysize.y + ")")
            .call(yAxis3).style('font-size',0.6*autofontsize+'px');
