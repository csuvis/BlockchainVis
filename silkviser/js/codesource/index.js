$(function () {	
	...
	var svgh = blockh;// 根据区块大小确定画布高度

	var blockspacemax = 0.6 * blockw;// 最大间距
	var blockspacemin = 0.3 * blockw;// 最小间距
	...
	function createsvg(svgdata,maxblockHeight,maxTrans) {
		...
		// 最大间距调优
		var spacesum = 0;
		for (let index = 0; index < svgdata.items.length - 1; index++) {//length-1计算前5个块就可以了
			spacesum = spacesum + blockspacetemp(svgdata.items[index].time_diff_lastblock);
		}
		//为了让第1和第6区块正好左右对齐，页面上允许总间距，总长-6个区块
		var spacepage = svgw - padding.left - padding.right - 6 * blockw;
		...
		var block5size = {// 第5个区块尺寸
			x: padding.left + blockw + blockspace(svgdata.items[4].time_diff_lastblock),
			y: padding.top,
			width: blockw,
			height: blockh,
			no: 5,
		...
		var block4size = {// 第4个区块尺寸
			x: padding.left + 2 * blockw + blockspace(svgdata.items[4].time_diff_lastblock) + blockspace(svgdata.items[3].time_diff_lastblock),
			y: padding.top,
			width: blockw,
			height: blockh,
			no: 4,
		...
		var block3size = {// 第3个区块尺寸
			x: padding.left + 3 * blockw + blockspace(svgdata.items[4].time_diff_lastblock) + blockspace(svgdata.items[3].time_diff_lastblock)
		...
		var block2size = {// 第2个区块尺寸
			x: padding.left + 4 * blockw + blockspace(svgdata.items[4].time_diff_lastblock) + blockspace(svgdata.items[3].time_diff_lastblock)
		...
		dwawChainDash(blockchaing, block6size, 4, svgdata);
		...
		var block1size = {// 第1个区块尺寸
			x: padding.left + 5 * blockw + blockspace(svgdata.items[4].time_diff_lastblock) + blockspace(svgdata.items[3].time_diff_lastblock)
				+ blockspace(svgdata.items[2].time_diff_lastblock) + blockspace(svgdata.items[1].time_diff_lastblock)
		...
		// =========生成链条================
		var chain65size = {
			x: block6size.x + blockw,
			y: block6size.y + 0.45 * blockh,
			w: blockspace(svgdata.items[4].time_diff_lastblock),
			h: blockh / 5
		}
	...
	function drawRect(svbobj, blockSize) {
		return svbobj.append('image').attr("opacity", blockSize.opacity).attr(
			'xlink:href', "/images/bg2.png").attr('x', blockSize.x).attr('y',
	...
	function drawBlockHead(group, blockNo, blockSize, data) {
		...
		var t1 = appendLineText(group, $.i18n.prop("svgHeight"), blockSize.x + 0.115
		...
		var t11 = appendLineText(group, data.items[blockNo - 1].height, blockSize.x + 0.15
			* blockw, blockSize.y + 0.25 * blockh, sizebarwidth, 0.3*sizebarheight);
		...
		var t2 = appendLineText(group, $.i18n.prop("svgBlockHash") + ': '
			...
			blockSize.x + 0.126 * blockw, blockSize.y + 0.4 * blockh,
		...
		var t3 = appendLineText(group, $.i18n.prop("svgPreviousBlockHash")
			...
			blockSize.y + 0.47 * blockh, 0.88 * blockw, 0.051 * blockh);
		...
		var t4 = appendLineText(group, $.i18n.prop("svgMerkleRoot") + ': '
			...
			blockSize.x + 0.126 * blockw, blockSize.y + 0.54 * blockh, 0.88 * blockw, 0.051 * blockh);
		...
		var t5 = appendLineText(group, $.i18n.prop("svgAge") + ': ' + agetime,
			blockSize.x + 0.126 * blockw, blockSize.y + 0.61 * blockh,
			0.88 * blockw, 0.051 * blockh);
		...
		var gbarbgsize = groupsize.append('rect').attr('class', 'barbgclass').attr('x',
			blockSize.x + 0.1 * blockw).attr('y',
				blockSize.y + 0.77 * blockh).attr('width', barwidth).attr(
		...
		var gbarfgsize = groupsize.append('rect').attr('class', 'barfgclass').attr('x',
			blockSize.x + 0.1 * blockw).attr('y',
				blockSize.y + 0.77 * blockh).attr('width', gper * barwidth).attr(
		...
		var gbartextsize = appendLineText(groupsize, $.i18n.prop("svgSize") + ': ' +blocksizeshow+' Bytes',
			blockSize.x + 0.12 * blockw, blockSize.y + 0.77 * blockh + 0.7 * barheight,
		...
		gbartextsize.attr("class", "blocktextblack");//改变显示的颜色
		...
		var gbartexttxs = appendLineText(grouptxs, $.i18n.prop("svgTrans") + ': ' + data.items[blockNo - 1].transaction_count,
			blockSize.x + 0.12 * blockw, blockSize.y + 0.69 * blockh + 0.7 * barheight,
			barwidth, 0.6 * barheight);
	...
	function dwawChainDash(blockchaing, blocksizeNo, no, svgdata) {
		...
		blockchaing.append("path").attr("stroke-dasharray", chaindashsizestr).attr("d", function () {
			var tempx = blocksizeNo.x + blockw + 0.03 * blockw;
	...
	function createtjbsvg(tjbdata) {
		...
		tjbsign.append("circle").attr("cx", tjbmainsvgpos.left + 4.0 * blockw).attr("cy", 0.038 * blockh)
			.attr('r',0.018*blockw).attr("class", "line3-text");
		tjbsign.append("text").attr("x", tjbmainsvgpos.left + 4.2 * blockw).attr("y", 0.05 * blockh)
			.attr("class", "legend").text($.i18n.prop("g-transaction_count"))
			.style('font-family','sans-serif')
			.style("font-size", 0.75*autofontsize + "px");
		...
		.attr("class", "bar1-rect").text("■").style("font-size", 0.75*autofontsize + "px");// reward_sum
		tjbsign.append("text").attr("x", tjbmainsvgpos.left + 2.8 * blockw).attr("y", 0.05 * blockh)
			.attr("class", "legend").text($.i18n.prop("g-reward_sum")).style('font-family','sans-serif')
			.style("font-size", 0.75*autofontsize + "px");
		...
		tjbmain.append("g")
			...
			.style("font-size", 0.6*autofontsize + "px");
		...
		tjbmain.append("g")
			...
			.call(tjby1Axis)
			.selectAll("text")
			.style("font-size", 0.65 * autofontsize + "px");
		...
		tjbmain.append("text")
			.attr("transform", "translate(" + (tjby3pos.x-0.15*blockw) + "," + 0.15 * blockh + ")")
			.text($.i18n.prop("axis-txs")).style('font-family','sans-serif').
			style("font-size", 0.75 * autofontsize + "px");
